...
class ProfileFeed extends Component {
    render() {
        ...
        const renderPosts = current_user_posts.map(post => {
            return <ErrorBoundary key={post.id}><Post key={post.id} post={post} users={users} likes={likes} comments={comments} current_user={current_user} resetAppState={resetAppState} /></ErrorBoundary>
        })
        return (
            <ErrorBoundary>
